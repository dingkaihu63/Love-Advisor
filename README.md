...
### 🚀 核心升级：前沿实证心理学扩展 (Core Updates)

在最新版本中，`Love-Advisor` 深度扩充了底层的知识图谱与决策逻辑，将理论模块从 10 个增加至 **14 个**，并同步升级了 AI Agent 的执行技能：

#### 1. 知识库深度扩张 (`knowledge_base.json`)
新增四个战略级模块，引入了 2024-2025 年心理学顶刊的元分析数据：

| 模块编码 | 核心研究内容 | 关键数据与理论支撑 |
| :--- | :--- | :--- |
| **Part 11** | **相似与互补效应** | [cite_start]相似吸引效应、**同质匹配(88.72%伴侣相似)**、音乐纽带模型、自我扩张理论、6×6 判断框架 [cite: 1] |
| **Part 12** | **场域心理学与约会** | [cite_start]**UGA 2025 环境实验**、4 大心理机制（吊桥/自我扩张/环境温暖）、按人格/依恋类型匹配选址策略 [cite: 2] |
| **Part 13** | **颜值进化信号分析** | [cite_start]光环效应元分析、匹配 vs. 交换假设、**短期 vs. 长期权重动态变化**、约会 App 双路径模型 [cite: 3] |
| **Part 14** | **关系接入动力学** | [cite_start]**朋友优先路径 (68%)**、弱关系理论、Gueguen 街头实验、社会渗透理论 [cite: 4] |

#### 2. AI 技能逻辑进化 (`SKILL.md`)
基于新增知识库，AI Agent 的分析维度实现了以下进化：
* [cite_start]**场景策略速查**：新增四个策略模块，支持针对不同社交情境提供垂直建议 [cite: 1, 2, 3, 4]。
* [cite_start]**多维适配分析**：在阶段二（侧写）中深度引入“相似性/互补性”考量，评估长期共振概率 [cite: 1]。
* [cite_start]**6×6 深度评估框架**：在阶段四新增第六项深度分析，从宏观维度透视关系健康度 [cite: 1]。
* [cite_start]**全时顾问模式**：阶段五新增三类针对性顾问服务（动态预警、场景话术、长期建模） [cite: 1, 2, 3, 4]。

#### 3. 开发者与执行规范 (Rules 16-20)
为确保逻辑严密性，系统新增了 5 条执行准则：
* [cite_start]**规则 16-20**：强制要求 AI 在输出建议时必须引用 14 个 Part 的量化结论，严禁脱离理论背景的感性猜测，并需实时标注结论的证据来源（如 [Part 11]） [cite: 1, 2, 3, 4]。
